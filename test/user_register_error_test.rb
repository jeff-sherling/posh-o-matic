...
class UserRegisterErrorTest < BaseTest
  ...
  def test_errors_with_empty_form_submit
    ...
    assert(error_page.email_error_present? &&
        error_page.password_error_present? &&
        error_page.confirm_password_error_present? &&
        error_page.accept_cbox_error_present?,
  ...
  def test_error_when_missing_name
    ...
    assert(error_page.error_alert_present?, 'Error alert should be present.')
  end
